package lucas.states.engine.menus.freeplay;
...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(WeekData.weeksList.length < 1)
		{
			...
			MusicBeatState.switchState(new states.ErrorState("NO WEEKS ADDED FOR FREEPLAY\n\nPress ACCEPT to go to the Week Editor Menu.\nPress BACK to return to Main Menu.",
				...
				function() MusicBeatState.switchState(new lucas.states.engine.menus.MainMenuState())));
			return;
		}
	...
	override function update(elapsed:Float)
	{
		...
		{
			scoreText.text = Language.getPhrase('personal_best', 'Score: {1} ({2}%)', [lerpScore, ratingSplit.join('.')]);
			positionHighscore();
		...
		if (controls.BACK)
		{
			...
			else 
			{
				...
				MusicBeatState.switchState(new lucas.states.engine.menus.MainMenuState());
			}
		}

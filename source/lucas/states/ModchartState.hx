...
import LuaClass.hxcGame;
import LuaClass.hxcWindow;
import LuaClass.hxcSprite;
import LuaClass.hxcCamera;
import LuaClass.hxcNote;
import LuaClass.hxcReceptor;
...
class ModchartState 
{
	...
	public function call(func:String, args:Array<Dynamic>, ?type : String):Dynamic {
		...
		try {
			...
			if (type != Lua.hxc_TFUNCTION) {
				if (type > Lua.hxc_TNIL)
	...
	function typeToString(type:Int):String {
		#if LUA_ALLOWED
		switch(type) {
			case Lua.hxc_TBOOLEAN: return "boolean";
			case Lua.hxc_TNUMBER: return "number";
			case Lua.hxc_TSTRING: return "string";
			case Lua.hxc_TTABLE: return "table";
			case Lua.hxc_TFUNCTION: return "function";
		}
		if (type <= Lua.hxc_TNIL) return "nil";
		#end
		return "unknown";
	}
	...
	function getErrorMessage(status:Int):String {
		...
		if (v == null || v == "") {
			switch(status) {
				case Lua.hxc_ERRRUN: return "Runtime Error";
				case Lua.hxc_ERRMEM: return "Memory Allocation Error";
				case Lua.hxc_ERRERR: return "Critical Error";
			}
	...
	function resultIsAllowed(leLua:State, leResult:Null<Int>) { //Makes it ignore warnings
		switch(Lua.type(leLua, leResult)) {
			case Lua.hxc_TNIL | Lua.hxc_TBOOLEAN | Lua.hxc_TNUMBER | Lua.hxc_TSTRING | Lua.hxc_TTABLE:
				return true;
		}
		return false;
	}
	...
	function getType(l, type):Any
	{
		return switch Lua.type(l,type) {
			case t if (t == Lua.hxc_TNIL): null;
			case t if (t == Lua.hxc_TNUMBER): Lua.tonumber(l, type);
			case t if (t == Lua.hxc_TSTRING): (Lua.tostring(l, type):String);
			case t if (t == Lua.hxc_TBOOLEAN): Lua.toboolean(l, type);
	...
	{	
		...
		if (PlayState.instance.Stage.isCustomStage && PlayState.instance.Stage.hxcArray.length >= 1)
		{
			for (i in PlayState.instance.Stage.hxcArray)
			{
				PlayState.instance.Stage.hxcArray.remove(i);
				i.stop();
			}
			PlayState.instance.Stage.hxcArray = [];
		}
		...
		//trace(PlayState.instance.Stage.hxcArray.length);
	}
    ...
    public function new(path:String, ?preloading:Bool = false, ?isStageLua:Bool = false)
    {
		...
		try{
			...
			if(resultStr != null && result != 0) {
				...
				if (isStageLua)
				{
					Stage.instance.hxcArray.remove(this);
					Stage.instance.hxcArray = [];
				}
				else
				{
					PlayState.instance.hxcArray.remove(this);
					PlayState.instance.hxcArray = [];
				}
		...
		else
		{
			...
			Lua_helper.add_callback(lua, "addLuaScript", function(luaFile:String, ?ignoreAlreadyRunning:Bool = false, ?traceMsg:Bool = true) { //would be dope asf. 
				...
				if(doPush)
				{
					...
					{
						for (luaInstance in PlayState.instance.hxcArray)
						{
					...
					PlayState.instance.hxcArray.push(new ModchartState(cervix)); 
					return;
				}
						...
						for (luaInstance in PlayState.instance.hxcArray)
						{
							if(luaInstance.scriptName == cervix)
							{
								//luaTrace('The script "' + cervix + '" is already running!');
									...
									PlayState.instance.hxcArray.remove(luaInstance); 
								return;
							}
						}
	
						for (luaInstance in PlayState.instance.Stage.hxcArray)
						{
							if(luaInstance.scriptName == cervix)
							{
								//luaTrace('The script "' + cervix + '" is already running!');
								
									PlayState.instance.Stage.hxcArray.remove(luaInstance); 
								return;
							}
						}
					}
			...
			Lua_helper.add_callback(lua, "getRunningScripts", function(){
				...
				for (idx in 0...PlayState.instance.hxcArray.length)
					runningScripts.push(PlayState.instance.hxcArray[idx].scriptName);
			...
			Lua_helper.add_callback(lua, "callScript", function(?luaFile:String, ?funcName:String, ?args:Array<Dynamic>){
				...
				var cervix = luaFile + ".hxc";
				if(luaFile.endsWith(".hxc"))cervix=luaFile;
				var doPush = false;
				#if MODS_ALLOWED
			...
			Lua_helper.add_callback(lua, "getGlobalFromScript", function(?luaFile:String, ?global:String){ // returns the global from a script
				...
				if(doPush)
				{
					for (luaInstance in PlayState.instance.hxcArray)
					{
						if(luaInstance.scriptName == cervix)
						{
							Lua.getglobal(luaInstance.hxc, global);

							var ret = Convert.fromLua(luaInstance.hxc, -1);
							Lua.pop(luaInstance.hxc,1); // remove the global
			...
			Lua_helper.add_callback(lua, "getNotes", function(y:Float)
			{
				...
				for (i in 0...PlayState.instance.notes.members.length)
				{
					...
					Lua.pushstring(lua, note.hxcNote.className);
					Lua.rawseti(lua, -2, i);
				}
			});
